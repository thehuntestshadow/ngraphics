...
const SharedUI = {
    ...
    /**
     * Render consistent empty state HTML
     * @param {string} type - 'history' | 'favorites' | 'search'
     * @param {string} [customMessage] - Optional custom message
     * @returns {string} HTML string
     */
    renderEmptyState(type = 'history', customMessage = null) {
        const icons = {
            history: '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
            favorites: '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
        ...
        const defaultMessages = {
            history: typeof i18n !== 'undefined' ? i18n.t('msg.noHistory', 'No history yet') : 'No history yet',
            favorites: typeof i18n !== 'undefined' ? i18n.t('msg.noFavorites', 'No favorites yet') : 'No favorites yet',
            search: typeof i18n !== 'undefined' ? i18n.t('msg.noResults', 'No results found') : 'No results found'
        };

        const message = customMessage || defaultMessages[type] || defaultMessages.history;
        const icon = icons[type] || icons.history;
...
const SharedDashboard = {
    ...
    _loadFromStorage(key) {
        try {
            ...
            console.warn(`SharedDashboard: Failed to parse ${key}, data may be corrupted:`, e.message);
            // Return empty but don't overwrite - let user know via console
            return [];
        }
    },
